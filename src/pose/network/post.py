...
from src.pose.preprocessing import denoise
...
def plot_pose(img_orig, joint_list, person_to_joint_assoc, bool_fast_plot=True, plot_ear_to_shoulder=False):
    ...
    for person_joint_info in person_to_joint_assoc:
        ...
        # modified to detected those who have more than valve joints
...
def decode_pose(img_orig, param, heatmaps, pafs, pose_val, plot_skeleton):
    # Bottom-up approach:
    ...
    # Only remain those well-detected people
    # update joint_list, person_to_joint_assoc
    ################################################################
    person_to_joint_assoc = denoise(person_to_joint_assoc, pose_val)
    # (Step 4): plot results
    if plot_skeleton:
